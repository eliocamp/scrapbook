...
CDR <- function() {
  ...
  icecdr::cdr_arctic_monthly(years, file = basename(file), 
                             dir = dirname(file)) |> 
    icecdr::cdr_fix()
...
arctic_d <- icecdr::sea_ice_index("north", "daily", dir = here::here("_data/arctic_regime/")) |> 
  fread() |> 
  _[year(time) < 2025] |> 
  _[, .(time, aice = extent/1e12)]

arctic <- arctic_d |> 
  _[, .(aice = mean(aice)), by = .(time = lubridate::floor_date(time, unit = "month"))] 
```
...
arctic |> 
  copy() |> 
  _[, aice := aice - mean(aice, na.rm = TRUE), by = .(month(time))] |> 
  ggplot(aes(time, aice)) +
  geom_line() +
...
breakpoint <- arctic |> 
  copy() |> 
  _[, .(aice = mean(aice, na.rm = TRUE)), by = .(year(time))] |> 
...
arctic |> 
  copy() |> 
  _[, Early := aice - mean(aice[year(time) %between% c(1990, 2000)], na.rm = TRUE), by = month(time)] |>
  _[, Late := aice - mean(aice[year(time) >= 2014], na.rm = TRUE), by = month(time)] |>
  _[, .(time, Early, Late)] |> 
  melt(id.vars = "time") |> 
...
The yearly mean logistic model predicts a right asymptote of about `r round(predict(model, newdata = data.frame(year = 10000)), digits = 1)` million squared kilometres.
...
temp_models <- rbind(
  t2m |> 
    ...
    _[, .(model = list(nls(t2m ~ glf(year, t0, left, right, rate),  
                                   control = nls.control(maxiter = 100),
                                   start = list(t0 = median(year), 
                                                left = min(t2m), 
                                                right = max(t2m), 
                                                rate = -0.2)))), 
      by = lat],
  t2m |> 
    _[, .(t2m = mean(t2m)), by = .(year(time), lat)] |>
    _[, year := year - min(year)] |> 
    _[lat <= 62, .(model = list(lm(t2m ~ year))), by = lat] 
)
...
sim_area <- sim_realistic |> 
  ...
  _[, .(aice = weighted.mean(aice, cos(lat*pi/180), na.rm = TRUE)), by = .(time)]
