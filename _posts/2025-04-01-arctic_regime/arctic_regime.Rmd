...
This week I read this new paper "Regime Shift in Arctic Ocean Sea-Ice Extent". 
...
The core of the argument is \@ref(fig:fig1), which compares a linear fit through the whole period with a linear fit until 2006 and then a constant line after. 

```{r fig1, fig.cap="Figure from https://example.com", out.extra="class=external"}
...
\@ref(fig:arctic-sep) shows the result plus linear fits for the period before and after 2007. 
...
Figure \@ref(fig:extent-daily) shows monthly extent anomalies computed using the whole period climatology (this will be important).  
...
The monthly timeseries after 2007 in Figure \@ref(fig:extent-daily) does look weird; the variability is through the roof. 
The reason is that the *seasonal cycle* changed significantly in 2007. 

```{r anomalies-periods}
arctic |> 
  copy() |> 
  _[, aice_minus := aice - mean(aice[year(time) %between% c(1990, 2000)], na.rm = TRUE), by = yday(time)] |>
  _[, aice_plus := aice - mean(aice[year(time) >= 2014], na.rm = TRUE), by = yday(time)] |>
  _[, .(time, aice_minus, aice_plus)] |> 
  melt(id.vars = "time") |> 
...
On the other hands, anomalies computed with the latter climatology look fine after 2007 but they swing before. 
...
```{r climatologies}
arctic_d |> 
  _[, .(aice = mean(aice, na.rm = TRUE)), by = .(time = update(time, year = 0), 
...
Take this extremely simlified equation for the evolution of sea ice concentration at a particular location: 
...
@paper does explore briefly what I would expect to be a better model: a [Generalised Logistic Function](https://en.wikipedia.org/wiki/Generalised_logistic_function). 
...
At least, it doesn't seem to be some extraordinary feature that "call for an explanation in terms of physical processes", as @paper puts it. 
...
> The fact that September Arctic SIE shows no trend during 2007–2024 may at first seem hard to explain. The Earth continues to warm, and the Arctic is warming faster than the global average.
...
t2m |> 
  ...
  scale_y_continuous("Annaul mean zonal mean temperature")
```
...
arctic_zonmean |> 
  ...
  _[, .(value = sum(value*cos(lat*pi/180))), by = .(variable, year)] |> 
  _[, line := predict(nls(value ~ glf(year, t0, left, right, rate), 
                          start = list(t0 = median(year), left = max(value), right = min(value), rate = -0.2))), by = .(variable)] |>
  ggplot(aes(year, value)) +
  geom_line(aes(color = variable)) +
  geom_line(aes(color = variable, y = line)) +
  theme(legend.position = "bottom") +
  labs(title = "September sea ice predicted from linear regression with annual temperature")
```
...
1. The evidence for a slowdown is much weaker than advertised. 
   The statistics are simply not super robust and depend on the specific month and dataset used (I haven't explored it here, but it also breaks down a bit when using area instead of extent. 
...
2. To the extent that there has been a slowdown, it doesn't necessarily represent a fundamental shift in Arctic sea ice behaviour. 
...
3. Alternative non-linear models naturally have different "regimes" but they don't necessarily "call for explanation in terms of physical processes". 

4. Independent of all these arguments, the observed slowdown could be simply explained by slower temperature increase in the northern polar regions in the recent decade. 
