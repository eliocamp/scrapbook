...
y <- sst[lat %~% c(0, -50) & lon %~% 150] |> 
  ...
  _[, pp_a := `-50` - `0`] |>
  # _[year(year) <= 2000, pp_a := `-50`] |>
  # _[year(year) > 2000, pp_a := `0` - `-50`] |>
...
W <- sst |> 
  ...
  _[y_std, on = "year"] |> 
  # _[year(year) > 2000] |> 
  _[, .(w  = cor(t_a, pp_a)^2 * sqrt(cos(lat*pi/180))), by = .(lon, lat)] 
...
e$left |> 
  ...
  # _[PC == "PC"] |> 
  ggplot(aes(lon, lat)) +
  geom_contour_fill(aes(z = t_a*sign*w)) +
